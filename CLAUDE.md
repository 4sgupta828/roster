...
## Prod / deploy (Railway) — NOT YET PROVISIONED

- Repo remote: **https://github.com/4sgupta828/roster**. No Railway project/prod URL exists for roster
  yet — the details in this section are inherited from eigen and are placeholders until roster's own
  Railway project + managed Postgres(pgvector) are provisioned. **Do not treat any `roster-api-*` URL
  as live until it is stood up and verified.**
- The deploy MECHANICS are reusable once provisioned: image = `deploy/Dockerfile`; deploy via
  `railway up` (uploads cwd + builds the Dockerfile — NOT wired to GitHub auto-deploy, so a `git push`
  does not redeploy; re-run `railway up -d -s <service>` after code changes; poll `railway service
  status` until `SUCCESS`).
- Ingest (prod-direct): `POST /admin/corpus/ingest` with `{"jobs":[{"connector","query","limit"}]}`
  and header `X-Admin-Token: $ROSTER_ADMIN_TOKEN`; watch `GET /corpus/queue`.
