"""Delete the eval / smoke throwaway accounts (intake-eval-%, briefs-smoke-%, rail-smoke-%, railcheck-% @roster.test) and their rows.
    ...
    async with cx.transaction():
        ...
        for t in TABLES:
            ...
            if not res.endswith(" 0"):
                print(" ", t, res)
        # maps are owned by a user but keyed on owner_id, not user_id (their revisions/reviews cascade)
        res = await cx.execute("delete from rs_map where owner_id::text = any($1::text[])", [str(i) for i in ids])
        if not res.endswith(" 0"):
            print("  rs_map", res)
